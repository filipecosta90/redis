...
dictType zsetDictType = {
    ...
    NULL,               /* key destructor - the B+ tree owns the element memory */
    NULL,               /* val destructor */
    NULL,               /* allow to expand */
    .no_value = 1,      /* no values stored (only element pointers) */
...
unsigned long zsetLength(const robj *zobj) {
    ...
    if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        length = ((const zset*)zobj->ptr)->tree->length;
    } else {
...
void zsetConvertAndExpand(robj *zobj, int encoding, unsigned long cap) {
    ...
    if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
         ...
         * than performing N independent O(log N) insertions. */
        unsigned long n = zsetLength(zobj);
        zbtElem **elems = n ? zmalloc(sizeof(zbtElem *) * n) : NULL;
        unsigned long cnt = 0;
...
robj *zsetDup(robj *o) {
    ...
    if (o->encoding == OBJ_ENCODING_LISTPACK) {
         ...
         * build the destination tree bottom-up in O(N) instead of performing N
         * independent O(log N) insertions. */
...
void zuiInitIterator(zsetopsrc *op) {
    ...
    if (op->type == OBJ_SET) {
        ...
        if (op->encoding == OBJ_ENCODING_LISTPACK) {
            ...
            it->sl.elem = zbtLast(it->sl.zs->tree, &it->sl.it);
        } else {
            serverPanic("Unknown sorted set encoding");
        }
    } else {
 ...
 *
 * Adding the elements one by one instead would make each of the n results pay a
 * root-to-leaf descent, a leaf scan dereferencing up to a leaf's worth of
 * elements, and a shift, plus a split and its linear parent fix-up once per
 * leaf filled. Packing the leaves in one linear pass needs the elements sorted,
 * which callers walking a single sorted-set source already provide: only
 * aggregating scores across several sources, or reading from a plain set, can
 * break the order. Confirming it costs one pass and skips the sort outright in
 * that common case; when the sort is needed it is the O(n log n) the
 * per-element inserts cost anyway. */
...
static void zdiffAlgorithm1(zsetopsrc *src, long setnum, size_t *maxelelen, size_t *totelelen,
    ...
    zbtElem *znode;
    sds tmp;
    /* Result elements in discovery order, handed to the tree in one build once
     * the walk is over. The difference cannot outgrow the set it starts from. */
    zbtElem **staged = zmalloc(sizeof(zbtElem *) * zuiLength(&src[0]));
    unsigned long staged_cnt = 0;
...
void zunionInterDiffGenericCommand(client *c, robj *dstkey, int numkeysIndex, int op,
    ...
    if (op == SET_OP_INTER) {
        ...
        if (zuiLength(&src[0]) > 0) {
            if (!cardinality_only)
                staged = zmalloc(sizeof(zbtElem *) * zuiLength(&src[0]));
         ...
         * For each element, either create a detached element (new) or update
         * its score in place (existing). Elements are inserted into the tree
         * in a single batch afterwards (Step 2). */
        for (i = 0; i < setnum; i++) {
            ...
            while (zuiNext(&src[i],&zval)) {
                ...
                if (link == NULL) {  /* if not exists */
                    ...
                    serverAssert((znode->enc & ZBT_SCORE_MASK) == ZBT_SCORE_DBL);
    ...
    int reverse)
{
    ...
    if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        /* If reversed, get the last node in range as starting point. */
        if (reverse) {
            ln = zbtNthInLexRange(t,range,-offset-1,NULL,&it);
        } else {
            ln = zbtNthInLexRange(t,range,offset,NULL,&it);
        }

        while (ln && limit--) {
            /* Abort when the node is no longer in range. */
            if (reverse) {
                if (!zslLexValueGteMin(zbtGetEle(ln),range)) break;
            } else {
                if (!zslLexValueLteMax(zbtGetEle(ln),range)) break;
            }
...
/* The order-statistic B+ tree used by large sorted sets has its own unit
 * tests and structural verification in zbtree.c (registered as the "zset"
 * REDIS_TEST and exposed via zbtreeTest). */
