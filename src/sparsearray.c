...
static inline uint32_t arSliceMaxIdx(arSlice *s) {
    if (s->encoding == AR_SLICE_DENSE) {
        return s->dense.max_idx;
    } else {
        return s->sparse.offsets[s->count - 1];
    }
}
...
void *arStringNew(const char *s, size_t len) {
     ...
     * hypothetical header corruption. */
    serverAssert(len < (1ULL << 63));
...
arSlice *arSliceDenseNew(redisArray *ar, uint32_t rel_idx, uint32_t slice_size) {
    ...
    s->count = 0;
    s->dense.offset = offset;
    s->dense.winsize = winsize;
    s->dense.max_idx = 0;
    s->dense.items = (void **)(s + 1);  /* Payload starts after struct */
    memset(s->dense.items, 0, winsize * sizeof(void *));
    return s;
}
...
void arSparseSetupPointers(arSlice *s) {
    char *p = (char *)(s + 1);
    size_t offsets_size = s->sparse.cap * sizeof(uint16_t);
    size_t padding = (sizeof(void *) - (offsets_size % sizeof(void *))) % sizeof(void *);
    s->sparse.offsets = (uint16_t *)p;
    s->sparse.values = (void **)(p + offsets_size + padding);
}
...
void arSliceFree(redisArray *ar, arSlice *s) {
    ...
    if (s->encoding == AR_SLICE_DENSE) {
        for (uint32_t i = 0; i < s->dense.winsize; i++) {
            arTrackValueOut(ar, s->dense.items[i]);
            arFreePtr(s->dense.items[i]);
        }
    } else {
...
arSlice *arSliceDenseGrowRight(redisArray *ar, arSlice *s, uint32_t rel_idx, uint32_t slice_size) {
    uint32_t new_winsize = s->dense.winsize;

    /* Double until rel_idx fits */
    while (rel_idx >= s->dense.offset + new_winsize && new_winsize < slice_size) {
        new_winsize <<= 1;
    }

    uint32_t new_offset = s->dense.offset;
    ...
    ns->count = s->count;
    ns->dense.offset = new_offset;
    ns->dense.winsize = new_winsize;
    ns->dense.max_idx = s->dense.max_idx;
    ns->dense.items = (void **)(ns + 1);

    /* Zero-fill first to ensure arIsEmpty() works for new slots, then
     * copy old data */
    memset(ns->dense.items, 0, new_winsize * sizeof(void *));
    uint32_t shift = s->dense.offset - new_offset;
    serverAssert(shift + s->dense.winsize <= new_winsize);
    memcpy(ns->dense.items + shift, s->dense.items, s->dense.winsize * sizeof(void *));
...
arSlice *arSliceDenseGrowLeft(redisArray *ar, arSlice *s, uint32_t rel_idx, uint32_t slice_size) {
    uint32_t old_end = s->dense.offset + s->dense.winsize;
    ...
    ns->encoding = AR_SLICE_DENSE;
    ns->count = s->count;
    ns->dense.offset = (uint32_t)new_offset;
    ns->dense.winsize = new_winsize;
    ns->dense.max_idx = s->dense.max_idx;
    ns->dense.items = (void **)(ns + 1);
    ...
    memset(ns->dense.items, 0, new_winsize * sizeof(void *));
    uint32_t shift = s->dense.offset - ns->dense.offset;
    serverAssert(shift + s->dense.winsize <= new_winsize);
    memcpy(ns->dense.items + shift, s->dense.items, s->dense.winsize * sizeof(void *));
...
arSlice *arSliceDenseGrowIfNeeded(redisArray *ar, arSlice *s, uint32_t rel_idx, uint32_t slice_size) {
    if (rel_idx >= s->dense.offset + s->dense.winsize) {
        return arSliceDenseGrowRight(ar, s, rel_idx, slice_size);
    } else if (rel_idx < s->dense.offset) {
...
arSlice *arSparsePromote(redisArray *ar, arSlice *s, uint32_t slice_size) {
    ...
    d->count = s->count;
    d->dense.offset = offset;
    d->dense.winsize = winsize;
    d->dense.max_idx = max_off;
    d->dense.items = (void **)(d + 1);

    /* Set the entries in the sparse representation into the
     * new dense slice. */
    memset(d->dense.items, 0, winsize * sizeof(void *));
    for (uint32_t i = 0; i < s->count; i++) {
        serverAssert(offsets[i] >= offset);
        serverAssert(offsets[i] - offset < winsize);
        d->dense.items[offsets[i] - offset] = values[i];
    }
...
arSlice *arDenseMaybeDemote(redisArray *ar, arSlice *d) {
    ...
    if (d->dense.winsize == AR_SLICE_MIN_ALLOC) return d; // Already small.
    ...
    size_t dense_bytes = arDenseAllocSize(d->dense.winsize);
    size_t sparse_bytes = arSparseAllocSize(ArraySparseKMin);
    if (d->dense.winsize < 4 * ArraySparseKMin) return d;
    ...
    s->encoding = AR_SLICE_SPARSE;
    s->count = 0;
    s->sparse.cap = ArraySparseKMin;
    arSparseSetupPointers(s);

    /* Copy every entry from dense to sparse. */
    uint16_t *offsets = s->sparse.offsets;
    void **values = s->sparse.values;
    for (uint32_t i = 0; i < d->dense.winsize && s->count < d->count; i++) {
        if (!arIsEmpty(d->dense.items[i])) {
            offsets[s->count] = d->dense.offset + i;
            values[s->count] = d->dense.items[i];
            s->count++;
        }
    }
...
void arDenseUpdateMaxIdx(arSlice *d, uint32_t deleted_off) {
    ...
    if (d->count == 0 || deleted_off < d->dense.max_idx) return;

    /* Scan backward from old max to find new max. */
    for (int pos = d->dense.max_idx - d->dense.offset; pos >= 0; pos--) {
        if (!arIsEmpty(d->dense.items[pos])) {
            d->dense.max_idx = d->dense.offset + pos;
            return;
        }
    }
}
...
arSlice *arSliceDup(redisArray *dup_ar, arSlice *s) {
    if (s->encoding == AR_SLICE_DENSE) {
        size_t sz = arDenseAllocSize(s->dense.winsize);
        arSlice *nd = zmalloc(sz);
        arTrackAlloc(dup_ar, nd);
        memcpy(nd, s, sizeof(arSlice));
        nd->dense.items = (void **)(nd + 1);
        memcpy(nd->dense.items, s->dense.items,
               s->dense.winsize * sizeof(void *));

        /* Duplicate arString pointers */
        for (uint32_t j = 0; j < s->dense.winsize; j++) {
            if (arIsPtr(nd->dense.items[j])) {
                nd->dense.items[j] = arStringDup(nd->dense.items[j]);
                arTrackValueIn(dup_ar, nd->dense.items[j]);
            }
        }
        return nd;
    } else {
        size_t sz = arSparseAllocSize(s->sparse.cap);
        arSlice *nsp = zmalloc(sz);
        arTrackAlloc(dup_ar, nsp);
        memcpy(nsp, s, sizeof(arSlice));
        arSparseSetupPointers(nsp);
        memcpy(nsp->sparse.offsets, s->sparse.offsets,
               s->sparse.cap * sizeof(uint16_t));
        memcpy(nsp->sparse.values, s->sparse.values,
               s->sparse.cap * sizeof(void *));

        /* Duplicate arString pointers */
        void **values = nsp->sparse.values;
...
void *arGet(redisArray *ar, uint64_t idx) {
    ...
    if (s->encoding == AR_SLICE_DENSE) {
        if (rel_idx < s->dense.offset ||
            rel_idx >= s->dense.offset + s->dense.winsize)
        {
            // The slice window does not include this index.
            return NULL;
        }
        return s->dense.items[rel_idx - s->dense.offset];
    } else {
        int found;
...
void arSet(redisArray *ar, uint64_t idx, void *v) {
    ...
    if (s->encoding == AR_SLICE_DENSE) {
        ...
        uint32_t pos = rel_idx - s->dense.offset;
        void *old = s->dense.items[pos];
        ...
        /* Update max_idx */
        if (rel_idx > s->dense.max_idx) {
            s->dense.max_idx = rel_idx;
        }
    } else {
        int found;
        uint32_t pos = arSparseFindPos(s, (uint16_t)rel_idx, &found);
        ...
        if (found) {
            ...
            if (s->count >= ArraySparseKMax) {
                ...
                uint32_t dpos = rel_idx - d->dense.offset;
                arTrackValueIn(ar, v);
                d->dense.items[dpos] = v;
                d->count++;
                ar->count++;
                if (rel_idx > d->dense.max_idx) d->dense.max_idx = rel_idx;
            } else {
                /* Insert in sparse */
                if (s->count >= s->sparse.cap) {
                    /* Grow capacity, we grow 2x but note that there is no
                     * point in growing more than kmax, so we clamp to kmax. */
                    uint32_t new_cap = s->sparse.cap * 2;
                    ...
                    ns->count = s->count;
                    ns->sparse.cap = new_cap;
                    arSparseSetupPointers(ns);

                    /* Copy old data to new slice */
                    uint16_t *old_offsets = s->sparse.offsets;
                    void **old_values = s->sparse.values;
                    uint16_t *new_offsets = ns->sparse.offsets;
                    void **new_values = ns->sparse.values;
...
int arDel(redisArray *ar, uint64_t idx) {
    ...
    if (s->encoding == AR_SLICE_DENSE) {
        if (rel_idx < s->dense.offset || rel_idx >= s->dense.offset + s->dense.winsize) {
            return 0;
        }
    ...
    if (s->encoding == AR_SLICE_DENSE) {
        ...
        if (eff_lo <= eff_hi) {
            ...
            for (uint32_t off = eff_lo; off <= eff_hi; off++) {
                uint32_t pos = off - s->dense.offset;
                if (!arIsEmpty(s->dense.items[pos])) {
                    arTrackValueOut(ar, s->dense.items[pos]);
                    arFreePtr(s->dense.items[pos]);
                    s->dense.items[pos] = NULL;
                    s->count--;
                    ar->count--;
                    deleted++;
            ...
            if (s->count > 0 && s->dense.max_idx >= eff_lo) {
                /* Scan backwards to find new max */
                s->dense.max_idx = s->dense.offset;
                for (int32_t i = (int32_t)win_hi; i >= (int32_t)win_lo; i--) {
                    if (!arIsEmpty(s->dense.items[i - s->dense.offset])) {
                        s->dense.max_idx = i;
                        break;
                    }
                }
 ...
 * differnet complexity properties, in case of big span of empty indexes. */
...
static arSlice *arDefragSlice(arSlice *s, unsigned long *work,
    ...
    if (s->encoding == AR_SLICE_DENSE) {
        for (uint32_t j = 0; j < s->dense.winsize; j++) {
            if (!arIsPtr(s->dense.items[j])) continue;
            void *new_ptr = defragfn(s->dense.items[j]);
            if (new_ptr) s->dense.items[j] = new_ptr;
        }
        if (work) {
            *work += s->dense.winsize;
            server.stat_active_defrag_scanned += s->dense.winsize;
        }
    } else {
        void **values = s->sparse.values;
        for (uint32_t j = 0; j < s->count; j++) {
