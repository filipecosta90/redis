   ...
   test "{$type} SCAN COUNT" {
    ...
    foreach enc {listpack skiplist} {
        test "{$type} ZSCAN scores match ZSCORE with encoding $enc" {
            ...
            # -1.2345678901234567e23 gives d2string()'s longest output (25 chars);
            ...
            # Insert after selecting the encoding: listpack normalizes -0 to 0.
            # Cover zero, integers inside and beyond the fast-path range,
            # and the smallest positive subnormal double.
            r zadd mykey 0 zero -0 negzero 1e18 largeinteger \
                9223372036854775808 integerboundary 5e-324 minsubnormal
            ...
            assert_equal 0 [dict get $res zero]
            if {$enc eq {skiplist}} {
                assert_equal -0 [dict get $res negzero]
            } else {
                assert_equal 0 [dict get $res negzero]
            }
            assert_equal 1000000000000000000 [dict get $res largeinteger]
            assert_equal 9223372036854776000 [dict get $res integerboundary]
            assert_equal 5e-324 [dict get $res minsubnormal]
